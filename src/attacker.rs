...
mod test {
    ...
    fn get_error(elements: Vec<DebugTraceElement>) -> Result<AttackError> {
        for trace in elements {
            if let DebugTraceElement::WithFailures {
                error,
                trace_elements,
                ..
            } = trace
    ...
    fn is_all_regular(trace_elements: &[DebugTraceElement]) -> bool {
        trace_elements
            .iter()
            .all(|t| matches!(t, DebugTraceElement::Regular { .. }))
    }
    ...
    fn test_attacker_receive_wrong_contract_address() -> Result<()> {
        ...
        assert_eq!(error, Error::WrongVictimAddress);
        Ok(())
    }
}
